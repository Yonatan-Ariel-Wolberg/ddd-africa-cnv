...
def write_vcf_file(output_file, ref_name, sorted_contig_lines, mutations_by_sample, sample_order, log_file=None):
    ...
    try:
        ...
        with open(output_file, 'w') as vcf_file:
            ...
            vcf_file.write("##source=CANOES_CSV_TO_VCF\n")
            ...
            for sample_id in sample_order:
                ...
                for mutation in mutations:
                    try:
                        ...
                        if safe_float(q_some) >= 80 and q_some != 'NA':
                            filter_status = 'PASS'
                        else:
                        ...
                        vcf_file.write(f"chr{chr_num}\t{start}\t.\t{ref}\t{alt}\t.\t{filter_status}\tEND={end};SVLEN={svlen}:SVTYPE=CNV;TOOL=CANOES;STRANDS={strand}\tGT:Q_SOME")
                            ...
                            if sample_id in mutations_by_sample:
                                ...
                                for sample_mutation in sample_mutations:
                                    ...
                                    if sample_mutation == mutation:
                                        vcf_file.write(f"\t{gt}:{safe_float(q_some) if q_some else '.'}")
                                        mutation_found = True
                                        break
...
def canoes_csv_to_bed(output_bed_file, mutations_by_sample, sample_order, log_file=None):
    """
...
def main():
    ...
    mutations_by_sample, sample_order = convert_canoes_csv_to_vcf(
    ...
    if mutations_by_sample is not None and sample_order is not None:
        ...
        canoes_csv_to_bed(args.bed_file, mutations_by_sample, sample_order, args.log_file)
