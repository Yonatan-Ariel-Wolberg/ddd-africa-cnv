...
def convert_clamms_bed_to_vcf(input_file, output_dir, combined_vcf_name, log_file, sample_file=None, fai_file=None):
    """
    ...
    # Return 'mutations_by_sample' and 'sample_order' as a tuple
    return mutations_by_sample, sample_order
...
def write_vcf_file(output_file, ref_name, sorted_contig_lines, mutations_by_sample, sample_order, log_file=None):
    ...
    try:
        ...
        with open(output_file, 'w') as vcf_file:
            ...
            vcf_file.write("##source=CLAMMS_BED_TO_VCF\n")
            ...
            vcf_file.write("##FILTER=<ID=LowQuality,Description=\"Low quality\">\n")
            ...
            for sample_id in sample_order:
                ...
                for mutation in mutations:
                    try:
                        ...
                        svlen = str(safe_int(end) - safe_int(start) + 1)

                        # Set filter status based on quality scores
                        if safe_float(q_some) >= 500 and safe_float(q_exact) > 0:
                            filter_status = 'PASS'
                        else:
                        ...
                        vcf_file.write(f"chr{chr_num}\t{start}\t.\t{ref}\t{alt}\t.\t{filter_status}\tEND={end};SVLEN={svlen};SVTYPE={cnv};TOOL=CLAMMS;STRANDS={strand}\tGT:Q_SOME")
...
def clamms_bed_to_bed(output_bed_file, mutations_by_sample, sample_order, log_file=None):
    """
    Function to convert CANOES CSV output to a .bed file.

    Parameters:
    - output_file: The path to the output .bed file.
    - mutations_by_sample: A dictionary containing the mutations for each sample in the order they appear in 'sample_order'.
    - sample_order: A list of sample IDs in the order they appear in the CLAMMS data.
    - log_file: The path to the log file to record errors and issues.
    """
    ...
    with open(output_bed_file, 'w') as bed_file:
            ...
            for sample_id in sample_order:
                ...
                for mutation in mutations:
                    try:
                        ...
                        # Write the mutation to the .bed file
                        bed_file.write(f"{chr_num}\t{start}\t{end}\t{sample_id}\t{svlen}\t{cnv}\t{q_some}\t{filter_status}\t{svmethod}\n")
        ...
        # Catches 'IOError' exceptions in the corresponding 'try' block, where 'e' is specifically the 'IOError' object               
            ...
            logging.error(f"Error writing VCF to {bed_file}: {e}")

        # Catches any other exceptions in the corresponding 'try' block, where 'e' is the exception object
        except Exception as e:
            logging.error(f"Unexpected error writing VCF to {bed_file}: {e}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Convert CLAMMS BED output to VCF format")

    # Add arguments to the parser for the input file, output directory, combined VCF file name, log file and sample_file
    parser.add_argument('-i', '--input', type=str, help='Input BED file', required=True)
    ...
    parser.add_argument('-b', '--bed_file', type=str, help='Name for the .bed file')
    ...
    mutations_by_sample, sample_order = convert_clamms_bed_to_vcf(
        args.input, args.output_dir, args.combined_vcf_name, args.log_file, args.sample_file, args.fai_file
    )
    
    # Ensure that mutations_by_sample and sample_order are not None
    if mutations_by_sample is not None and sample_order is not None:
        # Call the canoes_csv_to_bed function with the parsed arguments
        clamms_bed_to_bed(args.bed_file, mutations_by_sample, sample_order, args.log_file)
